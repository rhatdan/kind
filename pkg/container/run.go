...
// Run creates a container with "ContainreEngine run", with some error handling
...
func Run(image string, runArgs []string, containerArgs []string) (id string, err error) {
	...
	if len(output) < 1 {
		return "", fmt.Errorf("failed to get container id, received no output from %s run", Engine)
	}
